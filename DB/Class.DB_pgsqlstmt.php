 ...
 * @sa DB_PostgreSQL, DB
 */
class DB_pgsqlstmt extends DB_sqlstmt
{
    /*! @publicsection */
    /* ====================================================================== */
     ...
     */
    function DB_pgsqlstmt(&$dbmgr, $query)
    {
    ...
    /** 
     * Fetch the next row from this statement (DB_pgsqlstmt::execute must
     * be called first!).
     * 
     ...
     * (DB_pgsqlstmt::execute must be called first!).
     * 
     * @remarks Actually just a wrapper around PHP's
     ...
     * rows (ie. if DB_pgsqlstmt::fetchRow or
     * DB_pgsqlstmt::fetchAssoc has already been called to fetch all rows)
     * or if this statement doesn't have a valid result resource (ie. it hasn't
     * been executed yet).
     */
